...
final class CalorieEstimateService
{
    ...
    private function estimateWithClaudeWebSearchFallback(string $trimmed, string $apiKey, ?array $inputAnalysis = null): array
    {
        ...
        $match = $matchEvaluator->evaluate([
            ...
            'url' => $claudeResult['source_urls'][0] ?? '',
...
PROMPT;
    ...
    private function buildClaudeWebIdentificationPrompt(
        ...
        $queryInstructions = "- 最初の web_search は必ず次のクエリを使う: 「{$primary}」\n";
        if (is_string($alternate) && $alternate !== '' && $alternate !== $primary) {
...
- high: 標準的な仮定を置いても推定値がほとんど変わらない
  ...
  - high はゆで卵・白米など一般食品向け。市販・ブランド既製品には使わない
- medium: 標準的な仮定を置けば推定できるが、追加情報である程度カロリーが変わる
  ...
  - ブランド・メーカー・コンビニ・チェーン・宅配弁当などの市販既製品は、公式ラベルをこの推定では確認していないため最大 medium（商品名が具体的でも high にしない）
...
【市販・ブランド既製品の confidence】
- ナッシュ・コンビニ弁当・ファストフード・カップ麺・市販パッケージなど、公式ページで確定できる商品は AI 推定だけでは medium（または low）
- 「商品名が具体的」「自分がカロリーを知っている」だけでは high にしない
- このリクエストでは Web ページを確認していない前提で判定する（公式未確認のまま high は不可）
- 公式確認後に精度が上がり得るなら medium + should_offer_web_search=true が基本

【避ける判定】
- 標準量を仮定できるから high/medium にする
- 代表値を1つ提示できるから medium 以上にする
- 市販・ブランド品なのに商品名が具体的だから high にする
- 正確な商品ラベルがないため low（それだけでは不可。ただし市販既製品は high にせず medium）
...
- 標準仮定を使ったこと自体は confidence を下げる理由にしない（一般食品で安定していれば high 可）
- 市販・ブランド既製品は標準仮定が安定して見えても high にしない（公式ラベル確認で100kcal以上変わり得る）
...
- 市販・ブランド既製品なのに high にしていないか（していれば medium に下げる）
...
- ブランド・メーカー・店舗・具体的商品名があり、公式や商品ページで裏取りできそうなら、confidence が medium でも必ず true
...
- 知名度の高い市販品・チェーン商品・ブランド既製品は、AIがカロリーを知っていても true（公式情報の確認に有用）
...
【組み合わせの許容】
- medium / true（市販・ブランド・チェーン商品の基本。こちらを優先）
- high / false（ゆで卵・白米など一般食品のみ）
- medium / false
- low / true または false
- high / true は市販既製品では使わない（一般食品以外で high にしない）
confidence から機械的に決めない。特に「high だから false」にはしない

【判定の目安（校准用・個別例外処理ではない）】
- high になりやすい: ゆで卵2個、白米150g、牛乳200ml、バナナ1本（量が明確で、追加情報でも推定値が大きく変わらない一般食品）
- medium になりやすい: おにぎり1個、食パン1枚、味噌汁1杯、ナッシュ たらの辛旨チリソース、ローソン のり弁当、マクドナルド ビッグマック（市販・ブランド既製品は公式未確認のため medium）
- low になりやすい: 日替わり定食、定食、弁当、ラーメン、カレー、サラダ、パスタ
  （主菜・レシピ・店舗・ソース・セット内容などでおおよそ200kcal以上変わり得る。一人前を仮定できても low）
- should_offer_web_search=true の目安: カップヌードル シーフード、セブンイレブン 鮭おにぎり、ファミチキ、ザバス ミルクプロテイン、ほっともっと のり弁当、マクドナルド ビッグマック、ローソン のり弁当、ナッシュ たらの辛旨チリソース（いずれも confidence は medium）
...
- 一般食品は、公式ラベルが無くても推定値が安定していれば high、ある程度の幅なら medium、大きく振れるなら low（ラベル不足だけでは判定しない）
- 市販・ブランド既製品は公式未確認の AI 推定では最大 medium（high にしない）
...
- 市販・ブランド・チェーン商品の例: {"kcal": 整数, "confidence": "medium", "should_offer_web_search": true, "web_search_reason": "ブランド商品で公式ページ確認が可能"}
- 一般食品でWeb検索不要の例: {"kcal": 整数, "confidence": "high", "should_offer_web_search": false, "web_search_reason": "一般食品で商品ページ特定の意味が薄い"}
- 重量(g)が分かる場合は "assumed_weight_g": 整数 を追加してよい。カロリー値を明示できる場合は任意で "labeled_kcal": 整数 を追加してよい（kcal と同じ値）。labeled_kcal があっても、市販・ブランド品なら should_offer_web_search は true のまま（confidence は medium）
- 標準量を仮定した場合は任意で "assumption": "短い説明" を追加してよい
- 商品名が特定できた場合: 上記に "product_name": "正式な商品名" を追加
- labeled_kcal がある場合は kcal も必ず同じ値にする
- 公式ページに重量(g)が無い場合でも、標準量を仮定したなら assumed_weight_g を含めてよい
...
- 市販・ブランド既製品で confidence を high にしない（medium または low）
...
PROMPT;
    ...
    private function normalizeEstimate(array $json): ?array
    {
        ...
        // AI推定で Web 確認できそうな市販・ブランド品は high にしない
        if ($shouldOfferWebSearch && $confidence === 'high') {
            $confidence = 'medium';
        }
