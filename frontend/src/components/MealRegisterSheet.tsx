...
export function MealRegisterSheet({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      estimateCalories(trimmed)
        ...
        .catch((error: unknown) => {
          if (requestId !== estimateRequestId.current) return
          setEstimateError(error instanceof Error ? error.message : 'カロリー推定に失敗しました')
        })
        .finally(() => {
          if (requestId !== estimateRequestId.current) return
          setIsEstimating(false)
  ...
  return (
    <BottomSheet open={open} title={`${mealTitle}を記録`} onClose={onClose}>
      ...
      {isEstimating && (
        <div style={{ fontSize: 12, color: '#AAA', marginTop: 4 }}>カロリーを推定中...</div>
      )}
      {!isEstimating && estimateHint && !estimateError && (
        <div style={{ fontSize: 12, color: '#888', marginTop: 4 }}>{estimateHint}</div>
      )}
      {!isEstimating && estimateError && (
        <div style={{ fontSize: 12, color: '#C0392B', marginTop: 4 }}>{estimateError}</div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button
          ...
          style={{
            ...
            cursor: label.trim() && kcal.trim() && !isEstimating ? 'pointer' : 'not-allowed',
          }}
        >
          追加する
        </button>
